...
class LibraryRecommender:
    ...
    def load_and_preprocess(self):
        ...
        try:
            ...
            print(f"**********\nDEBUG: Columns initial: {self.df.columns.tolist()}\n**********")
            
            # Patch weird columns
            if 'Ratingpies' in self.df.columns:
                 print("DEBUG: Patching 'Ratingpies' -> 'Rating'")
                 self.df.rename(columns={'Ratingpies': 'Rating'}, inplace=True)
            
            # Check for Title
            if 'Title' not in self.df.columns:
                print("DEBUG: 'Title' column missing! Available: ", self.df.columns.tolist())
                # Try to fuzzy match or hard fail
            ...
            # We strictly check if they are in the columns list to avoid ambiguity
            has_dept = 'Department' in self.df.columns
            has_rating = 'Rating' in self.df.columns
            
            print(f"DEBUG: has_dept={has_dept}, has_rating={has_rating}")
            
            if has_dept and has_rating:
                print("Detected pre-processed dataset. Skipping inference.")
                try:
                    self.df['Department'] = self.df['Department'].fillna('General').astype(str)
                    self.df['Rating'] = pd.to_numeric(self.df['Rating'], errors='coerce').fillna(3.5)
                except Exception as e:
                    print(f"Error processing existing columns: {e}. Re-running inference.")
                    has_dept = False 
            
            if not has_dept or not has_rating:
                # Infer Department
    ...
    def get_top_50_by_dept(self, dept_name):
        ...
        if 'Department' not in self.df.columns:
            print("CRITICAL WARNING: 'Department' column missing in get_top_50_by_dept. Re-running inference.")
            self.df['Department'] = self.df['Title'].apply(self._infer_department).fillna('General')
            self.df['Rating'] = self.df.apply(self._calculate_quality_rating, axis=1)
